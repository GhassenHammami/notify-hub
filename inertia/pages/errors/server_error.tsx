...
const ServerError: InertiaPage<ServerErrorProps> = ({ error, message, code, title, details }) => {
  const errorMessage =
    message || error || "Something went wrong on our end. We're working to fix it."
  const errorCode = code || '500'
  const errorTitle = title || 'Server Error'
      ...
      <div className="flex flex-1 items-center justify-center bg-gray-50 px-4 py-12">
        <div className="mx-auto max-w-2xl text-center">
          <div className="mb-8">
            ...
            <h2 className="mb-4 text-2xl font-semibold text-gray-700">{errorTitle}</h2>
          ...
          {errorDetails && (
            <div className="mb-8 rounded-2xl bg-white p-6 shadow-lg">
              ...
              <div className="rounded-lg bg-gray-50 p-4 text-left">
                <code className="text-sm text-gray-700">{errorDetails}</code>
              </div>
            </div>
          )}
